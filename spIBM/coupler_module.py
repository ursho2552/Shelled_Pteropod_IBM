...
def Calculate_sunrise_sunset(lats,lons,year,current_time,rising_time=True,setting_time=False,localOffset=-7,backwards=False):
    ...
    The sunrise is calculated for the next day. The shift in one day is done since the simulation time step is one day,
    and the origin is set at noon. Description of algorithm is found in: http://www.edwilliams.org/sunrise_sunset_algorithm.htm
    ...
    sinDec = 0.39782 * np.sin(np.pi/180 * L)
    cosDec = np.cos(np.arcsin(sinDec))

    zenith = 90.83333 #official
    cosH = (np.cos(np.pi/180 * zenith) - (sinDec * np.sin(np.pi/180 * latitude))) / (cosDec * np.cos(np.pi/180 * latitude))
    cosH_above = cosH > 1
    cosH_below = cosH < -1
    ...
    while cosH_outside_range.sum() > 0:
        ...
        latitude[cosH_outside_range*(negative_lat==True)] += 0.1
        latitude[cosH_outside_range*(negative_lat==False)] -= 0.1
        cosH = (np.cos(np.pi/180 * zenith) - (sinDec * np.sin(np.pi/180 * latitude))) / (cosDec * np.cos(np.pi/180 * latitude))
        cosH_outside_range = (cosH > 1) + (cosH < -1)
    ...
    localT = np.where(localT < 0, 24 + localT, localT)
    seconds_since_origin = (tmp_datetime-origin).total_seconds() + localT*60*60

    return seconds_since_origin

def prepare_particles(pset,fieldset,year,w_down=45,w_up=45,w_down_wings=14,max_size=4.53,max_depth=250.0,backwards=False):
    """This function calculates the variable values used for the DVM movement, i.e. the downward and upward swimming speeds, as well as 
    the timings of the onset of the vertical migration. This function also resets binary values used to keep track of their position more easily.
...
#    dep_time_dist = np.random.normal(20, 20, pset.size)
    ...
    sunset_seconds = Calculate_sunrise_sunset(lats,lons,year,current_time,rising_time=0,setting_time=1,localOffset=-7,backwards=backwards)
    sunrise_seconds = Calculate_sunrise_sunset(lats,lons,year,current_time,rising_time=1,setting_time=0,localOffset=-7,backwards=backwards)
...
def update_particleset(matrix,pset,fieldset,pclass,year,day,die_list,max_size=4.53,backwards=False):
    """This function translates the values from a numpy array to a parcels object
    In addition, it calculates the swimming velocities, onset of vertical migration and DVM depths based on the size of the pteropods.
    Function also adds eggs to the custom parcels object.
    Binary and cumulative values used to keep track of their exposure, depth, location of maximum NPP are reset.
    ...
    Returns:
    pset (parcels obj): custom parcels object containing pteropods with updated values (state variables and variables used to calculate movement)
    UHE 5/10/2020
...
#    idx_newcomers = np.squeeze(not np.isin(IDs_mat,IDs_pset))
    ...
    # add the newcomers
#    print('Number of new eggs: {}'.format(idx_newcomers.sum()))
    ...
    pset.add(ParticleSet(fieldset=fieldset, pclass=pclass,\
                                    lon=pset.particle_data['lon'][idx_parents], lat=pset.particle_data['lat'][idx_parents],\
                                    depth=pset.particle_data['min_depth'][idx_parents],\
                                    time=pset.particle_data['time'][idx_parents],stage=matrix[idx_newcomers,2],\
                                    generation=pset.particle_data['generation'][idx_parents]+1,\
                                    MyID=matrix[idx_newcomers,0],Parent_ID=matrix[idx_newcomers,9],\
                                    Parent_shell_size=matrix[idx_newcomers,10],lonlatdepth_dtype=np.float32))
        

    #reset DVM min depth seeking
    pset.particle_data['chl_max'][:] = -99999.9
    pset.particle_data['chl_ascent'][:] = -99999.9
    pset.particle_data['depth_chl_max'][:] = 0.0#99999.9
    
    return pset
    
    
def get_dead_particles(pset,die_list):
    """This function removes a given particle from the cutom parcels object based on their indeces given in a list
