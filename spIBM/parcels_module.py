...
class PteropodParticle(JITParticle):
    """Definition of the attributes and additional characteristics of the
    pteropod particle.
...
def ReturnToSurface(particle, fieldset, time):
    """This function handels "through surface errors" by pushing particles
    back to below the surface
...
def PushToWater(particle, fieldset, time):
    """This function handels "through boundary errors" by pushing particles
    back to the ocean if beached
...
def pteropod_kernel(particle, fieldset, time):
    """This function defines the movement and interaction of pteropds with the
    environment. (In the future, this function should be separated into
    advection, DVM, interaction)
